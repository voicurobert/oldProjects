...
public class SingularFloods2{
    ...
    public SingularFloods2( double floodLifeTime, double raiseTime, double maxFlow, double gamma, double step, int raiseIterNr, int decreaseIterNr ){
    ...
    public void calculateSingularFloods(){
        ...
        Log.log( "Volume: " + dimensionlessVolume );
        while( true ){
            A += 0.01;
            squareRootBase = calculateSquareRootBase( A );
            squareRoot = Math.sqrt( squareRootBase );
            calculateFirstArgument( A );
            calculateSecondArgument( A );
            calculateThirdArgument( A );
            calculatedVolume = ( firstArgument + secondArgument * thirdArgument ) - dimensionlessTime/A;
            if( Math.abs( dimensionlessVolume - calculatedVolume ) < 0.001 ){
                this.A = A;
                calculateFlowValues();
                break;
            }
