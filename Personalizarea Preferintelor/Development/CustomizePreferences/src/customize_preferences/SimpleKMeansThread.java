...
public class SimpleKMeansThread implements Runnable {
    ...
    private void recalculateCentroids(){
        ...
        for (int centroidIndex = 0; centroidIndex < centroids.size(); centroidIndex++) {
            ...
            if (clusterAllocation.containsKey(centroidIndex)) {
                ...
                for (int i = 0; i < 5; i++) {
                    newValues[i] = roundValue( calculateNewCentroidData(i, allocatedClients) );
                }
                centroid.setNumericVector(newValues);
                centroids.add(centroidIndex, centroid);
               // System.out.println("recalculated: " + centroid.toString());
                centroids.remove(centroidIndex + 1);
                FrameInstanceHolder.instance.getSimpleKMeansFrame().setTableData(centroids);
            }
        }
    }
    ...
    private double calculateNewCentroidData( int index, List<Client> allocatedClients ){
        ...
        for( Client client : allocatedClients ){
            switch( index ){
                case 0 : result += client.getCriteriu4();
                    break;
                case 1 : result += client.getCriteriu5();
                    break;
                case 2 : result += client.getCriteriu6();
                    break;
                case 3 : result += client.getCriteriu7();
                    break;
                case 4 : result += client.getCriteriu8();
                    break;
                default : result += 0.0;
                    break;
            }
    ...
    private double euclidianDistance( Client client, Centroid centroid ){
        double result = 0.0;
        double[] clientValues = client.getNumericVectorValues();
        for( int i = 0; i<5; i++ ){
            result += (double) Math.abs( Math.pow( clientValues[i] -  centroid.getNumericVector()[i] , 2 ) );
        }
        // add result from levenshetein distance
        result += levenshteinDistance(client, centroid);
        return result;
    }
    ...
    private double manhattanDistance( Client client, Centroid centroid ){
        ...
        double[] clientValues = client.getNumericVectorValues();
        for( int i = 0; i<5; i++ ){
            result += (double) Math.abs( clientValues[i] -  centroid.getNumericVector()[i] );
        }
        result += levenshteinDistance(client, centroid);
        return result;
    }
    ...
    private double chebyshevDistance( Client client, Centroid centroid ){
        ...
        double[] clientValues = client.getNumericVectorValues();
        for( int i = 0; i<5; i++ ){
            result[ i ] = (double) Math.abs( clientValues[i] -  centroid.getNumericVector()[i] );
        }
        result[5] = levenshteinDistance(client, centroid);
        Arrays.sort( result );
        double x = result[5];
        return x;
    }
    ...
    private double minkowskiDistance( Client client, Centroid centroid ){
        ...
        double[] clientValues = client.getNumericVectorValues();
        for( int i = 0; i<5; i++ ){
            result += (double) Math.abs( Math.pow( ( clientValues[i] - centroid.getNumericVector()[i]) , 3 ) );
        }
        result += levenshteinDistance(client, centroid);
        return (double) Math.pow( result, 1/3 );
    }
    
    public int levenshteinDistance( Client client, Centroid centroid ){
        int[] distance = new int[3];
        String[] clientValues = client.getStringVectorValues();
        String[] centroidValues = centroid.getStringVector();
        for( int i = 0; i < 3; i++ ){
            String clientString = clientValues[i];
            System.out.println(clientString);
            String centroidString = centroidValues[i];
            System.out.println(centroidString);
            int charsNrToAdd = 0;
            if( clientString.length() == centroidString.length() ){
                // same string size, check the differences of strings 
                distance[i] = getNrOfDiffChars( clientString, centroidString, centroidString.length() );
            }else{
                if( clientString.length() > centroidString.length() ){
                    charsNrToAdd = clientString.length() - centroidString.length();
                    distance[i] = getNrOfDiffChars( clientString, centroidString, centroidString.length() );
                    distance[i] = distance[i] + charsNrToAdd;
                }else{
                    charsNrToAdd = centroidString.length() - clientString.length();
                    distance[i] = getNrOfDiffChars( clientString, centroidString, clientString.length() );
                    distance[i] = distance[i] + charsNrToAdd;
                }
            }
        }
    ...
    private int getNrOfDiffChars( String s1, String s2, int length ){
        int nrOfDiffChars = 0;
