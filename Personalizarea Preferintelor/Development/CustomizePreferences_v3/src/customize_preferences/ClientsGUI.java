...
public class ClientsGUI extends javax.swing.JFrame {
    ...
    private List<AddClientGUI> instantiatedClientFrames;
    ...
    public ClientsGUI() {
        ...
        instantiatedClientFrames = new ArrayList<>();
    ...
    public List<AddClientGUI> getInstantiatedClientFrames() {
        return instantiatedClientFrames;
    }
    ...
    public void refreshClientsTable(){
        if( DatabaseManager.instance.isConnected() ) {
            ...
            JOptionPane.showMessageDialog(this, "Eroare la conexiunea cu baza de date");
        }
    }
    ...
    private void addClientButtonClicked(){
        int size = instantiatedClientFrames.size();
        int id = 0;
        String prefix = "Personalizare preferinte Client_";
        String title = "";
        int dbClientId = DatabaseManager.instance.getLastInsertedClient();
        
        if( dbClientId == 99 || size == 99 ){
            JOptionPane.showMessageDialog(this, "S-a atins numarul maxim permis pentru personalizarea clientilor!");
            return;
        }
        
        if( dbClientId == 0 && size == 0 ){
            id = size + 1;
            title = prefix + id;
        }else if( dbClientId != 0 && size == 0 ){
            size = dbClientId;
            id = size + 1;
            title = prefix + id;
        }else if( dbClientId == 0 && size != 0 ){
            ...
            AddClientGUI clientGui = instantiatedClientFrames.get(size - 1 );
            id = clientGui.getFrameId() + 1;
            title = prefix + id;
        }
       
        AddClientGUI clientFrame = new AddClientGUI( id );
        clientFrame.setTitle( title );
        instantiatedClientFrames.add( clientFrame );
    ...
    private void sentToProviderButtonClicked(){
        JOptionPane.showMessageDialog( this,"Preferinta(ele) a fost trimisa catre Agentul Furnizor" );
